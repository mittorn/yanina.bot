...
def video(p,t,m):
	...
	res = vk_call(CALL_NORMAL,'video.search',{'adult':1,'q':t,'count':10})
...
def film(p,t,m):
	...
	res = vk_call(CALL_NORMAL,'video.search',{'adult':1,'q':t,'count':10,'filters':'long'})
	attachments = ','.join([ 'video'+str(i['owner_id'])+'_'+str(i['id']) for i in res['items']])
	vk_send(p,'Фильмы по вашему запросу',attachments)

def pron(p,t,m):
	"cmd порно,порево,порище Поиск порно"
	res1 = vk_call(CALL_NORMAL,'video.search',{'adult':1,'q':t,'count':200,'filters':'mp4'})
	res2 = vk_call(CALL_NORMAL,'video.search',{'adult':0,'q':t,'count':200,'filters':'mp4'})
...
def photo(p,t,m):
	...
	res = vk_call(CALL_NORMAL,'photos.search',{'q':t,'count':10})
...
def docs(p,t,m):
	...
	res = vk_call(CALL_NORMAL,'docs.search',{'q':t,'count':10})
...
def gif(p,t,m):
	...
	res = vk_call(CALL_NORMAL,'docs.search',{'q':t +' ' +'.gif','count':200})
